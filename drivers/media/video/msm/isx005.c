...
static int isx005_reg_preview(void)
{
	...
	if(prev_af_mode == FOCUS_MACRO){
		...
		isx005_i2c_write(isx005_client->addr, 0x4852, 0x4402, WORD_LEN);		
		isx005_i2c_write(isx005_client->addr, 0x4850, 0x01, BYTE_LEN);
	}
...
static int isx005_check_focus(int *lock)
{
	...
	int result_macro_AF_1=0, result_macro_AF_2=0;
	unsigned short macro_AF_1=0, macro_AF_2=0;
	...
	result_macro_AF_1 = isx005_i2c_read(isx005_client->addr, 0x6D7A, &macro_AF_1, WORD_LEN);
	
	if(result_macro_AF_1<0){
		printk("isx005_macro_AF_1 read fail");
		return rc;
	}
	
	result_macro_AF_2 = isx005_i2c_read(isx005_client->addr, 0x4876, &macro_AF_2, WORD_LEN);
...
int isx005_sensor_release(void)
{
	...
	{
		CDBG("==========================isx005_sensor_not_release sensorAlwaysOnTest is true ==========================");
		printk("==========================isx005_sensor_not_release sensorAlwaysOnTest is true ==========================");
		return 0;
	}
	else{
		CDBG("==========================isx005_sensor_release sensorAlwaysOnTest is false ==========================");
